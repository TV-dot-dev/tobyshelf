...
Tobyshelf supports **Forward Auth**, allowing you to specify when a user is logged in using a reverse proxy and existing SSO provider.

## ⚠️ Security

**Important**: Enabling forward auth means Tobyshelf will **fully trust headers sent by the reverse proxy**. Never expose Tobyshelf directly to the internet when using forward auth - always route through your authenticated proxy, otherwise outsiders can attempt to impersonate any username they know about.

## Configuration

Provide Tobyshelf with the following environment variables:
...
services:
  tobyshelf:
    image: ghcr.io/tobyshelf-tools/tobyshelf:v0.38.2
    environment:
...
1. **Access Admin Settings**: Log in to Tobyshelf as an admin user
