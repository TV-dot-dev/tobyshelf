...
Tobyshelf provides a Komga-compatible API that allows you to use Komga clients (like Tachiyomi, Tachidesk, Komelia, etc.) to access your Tobyshelf library.

## Features

The Komga API implementation in Tobyshelf provides the following endpoints:
...
Tobyshelf organizes books differently than Komga:

- **Komga**: Libraries → Series → Books
- **Tobyshelf**: Libraries → Books (with optional series metadata)
...
1. Navigate to **Settings** in Tobyshelf
...
1. Create an OPDS user account in Tobyshelf settings
...
3. Configure the source:
   - Server URL: `http://your-tobyshelf-server/`
...
1. Install Komelia
2. Add a server:
   - URL: `http://your-tobyshelf-server/`
   - Username: Your OPDS username
   - Password: Your OPDS password
...
- Read progress tracking from Komga clients is not synchronized with Tobyshelf
...
- Ensure the Komga API is enabled in Tobyshelf settings
...
- The Komga API uses OPDS user accounts, not your main Tobyshelf account
- Create an OPDS user in the Settings → OPDS section
- Use those credentials in your Komga client

## API Compatibility

The Tobyshelf Komga API aims to be compatible with Komga v1.x API. While not all endpoints are implemented, the core functionality needed for reading and browsing is supported.
